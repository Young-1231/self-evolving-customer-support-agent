#!/usr/bin/env python
"""Exp E_v2 — v2.7 merged-answer guardrail (Subagent + Handoff, fixed).

Background
----------
Exp E ran the multi-specialist router in two extremes:

  * mode='core'  (no per-sub guardrail): multi_intent res = 46.8%,
    overall escalation = 36.6%, but the merged answer is never guarded
    (output PII could leak, hallucination unchecked).
  * mode='observed' (full per-sub guardrail): multi_intent res = 0%,
    overall escalation = 85.2%.  Per-sub groundedness fires on every
    sub-answer; one borderline sub-judge -> whole ticket escalates.

v2.7 fix
--------
Specialists run mode='core' (raw sub-answer).  Orchestrator merges, then
calls ``GuardrailPipeline.check_output(merged_answer, union_ctxs)`` ONCE.
We expect multi_intent res > 30% AND overall escalation < 75% — i.e. we
recover most of the multi_intent gain while keeping guardrail discipline.

Apples-to-apples vs Exp D: same KB, same tickets, same calibrator,
same LLM-judge groundedness, same balanced PII.  The ONLY change vs
Exp E (core) is: orchestrator-level single guardrail on merged answer.
...
def _comparison_row(name: str, summary):
    ...
    mi_n = mi.get("n", 0)
    return f"  {name:18s} : esc={_fmt_pct(esc)}  multi_intent_res={_fmt_pct(mi_res)} (n={mi_n})  p50={p50:.0f}ms"
...
def main():
    ...
    ap.add_argument("--workdir", default=os.path.join(here, "..", "experiments", "stress_test_expanded", "exp_e_v2"))
    ...
    # Single guardrail instance shared by both:
    #   - the base SupportAgent (so single-intent paths stay guarded)
    #   - the orchestrator (so merged-mode runs ONE check_output)
    ...
    print(f"[exp-e-v2] kb={len(kb)} tickets={len(tickets)} concurrency={args.concurrency}")
    print(f"[exp-e-v2] groundedness=LLM-judge({cfg.model})  judge_tau={args.judge_tau}")
    print(f"[exp-e-v2] pii_mode={args.pii_mode}  calibrator_domains={list(cal.domain_thresholds.keys())}")
    print(f"[exp-e-v2] orchestrator=MultiAgentOrchestrator(guardrail_mode='merged')")
    print(f"[exp-e-v2] specialists=[billing,account,technical,refund,general]  mode='core'")

    tracer = Tracer(workdir=args.workdir, filename="stress_trace.jsonl")
    open(tracer.path, "w").close()

    # Single shared router (cheap deepseek-chat call, cache hits across workers).
    router_backend = build_backend(cfg)
    router = IntentRouter(backend=router_backend, cache=not args.no_router_cache)

    def agent_factory():
        backend = build_backend(cfg)
        # Base SupportAgent: guarded (so single-intent fast path still runs
        # the full Exp-D pipeline).  Specialists wrap this base but in
        # mode='core' (raw answer) for the merged-mode multi-intent path.
        ...
        # v2.7: guardrail_mode='merged' = orchestrator runs ONE check_output
        # on the merged answer + union(contexts), instead of N per-sub guards.
    ...
    # 7-column comparison: Original / Exp B / Exp C / Exp D / Exp E (core) / Exp E (observed) / Exp E_v2 (merged)
    print("\n=====  Comparison: Original / B / C / D / E (core) / E (observed) / E_v2 (merged)  =====")
    paths = {
        ...
        "Exp E_v2 (merged)": os.path.join(args.workdir, "load_summary.json"),
    }
    summaries = {name: _load_summary(p) for name, p in paths.items()}
    for name in ("Original", "Exp B", "Exp C", "Exp D", "Exp E (core)", "Exp E (observed)", "Exp E_v2 (merged)"):
    ...
    print(f"\n[exp-e-v2] summary -> {os.path.join(args.workdir, 'load_summary.json')}")
    print(f"[exp-e-v2] report  -> {os.path.join(args.workdir, 'report.md')}")


def _write_report_md(workdir, summaries, router_stats, wallclock_s):
    rows = []
    ordered = ("Original", "Exp B", "Exp C", "Exp D",
    ...
    e_v2 = summaries.get("Exp E_v2 (merged)") or {}
    e_v2_mi = (e_v2.get("by_category") or {}).get("multi_intent") or {}
    success_mi = e_v2_mi.get("resolution_rate", 0.0) > 0.30
    success_esc = e_v2.get("escalation_rate", 1.0) < 0.75
    success_overall = success_mi and success_esc
    ...
    md = [
        ...
        *rows,
        ...
        f"({100*e_v2_mi.get('resolution_rate', 0):.1f}%)",
        f"- overall escalation < 75%: **{'PASS' if success_esc else 'FAIL'}** "
        f"({100*e_v2.get('escalation_rate', 0):.1f}%)",
