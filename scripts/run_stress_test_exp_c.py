#!/usr/bin/env python
"""Exp C: re-run Exp B tickets with calibrator + tuned PII precision.

Goal: validate whether per-domain threshold calibration + PII precision mode
'balanced' actually fix the bottlenecks identified in §4g (escalation 92%,
block 19.8% in Exp B).

Reads:
  - data/kb_expanded/index.jsonl                      (same KB as Exp B)
  - experiments/stress_test_expanded/exp_b/tickets.jsonl
  - experiments/calibration/thresholds.json
Writes:
  - experiments/stress_test_expanded/exp_c/{load_summary.json, report.md, ...}
"""
...
def main():
    ...
    ap.add_argument("--workdir", default=os.path.join(here, "..", "experiments", "stress_test_expanded", "exp_c"))
    ap.add_argument("--concurrency", type=int, default=16)
    ap.add_argument("--pii-mode", default="balanced", choices=["strict", "balanced", "relaxed"])
    args = ap.parse_args()
    ...
    print(f"[exp-c] kb={len(kb)} tickets={len(tickets)} concurrency={args.concurrency} "
          f"pii_mode={args.pii_mode} calibrator_domains={list(cal.domain_thresholds.keys())}")
    ...
    with open(os.path.join(args.workdir, "load_records.jsonl"), "w", encoding="utf-8") as f:
        for r in records:
            from dataclasses import asdict; f.write(json.dumps(asdict(r), ensure_ascii=False) + "\n")
    summary = summarize_load(records)
    with open(os.path.join(args.workdir, "load_summary.json"), "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    print(f"\n=====  Exp C (calibrated + pii_mode={args.pii_mode})  =====")
    ...
    print(f"\n[exp-c] summary -> {os.path.join(args.workdir, 'load_summary.json')}")
