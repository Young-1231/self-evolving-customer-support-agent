...
### 4i. Exp D：LLM-judge groundedness 终于击中真瓶颈（3 轮证伪→第 4 次成功）

§4h 锁定真瓶颈后，按 `design/NEXT_PHASE_P1.md` 完整实施 P1：
`src/seagent/guardrails/groundedness_llm.py`（LLM-as-judge 二分类，DeepSeek 单次调用，
soft-fail 到 neutral）+ pipeline 注入点 + `EscalationVoter`（三信号 vote 模块）+
`scripts/run_stress_test_exp_d.py`。

Exp D 配置：扩展 KB(176 篇) + Exp B mixed tickets(500) + 校准 thresholds + balanced PII +
**LLM-judge groundedness 替换 n-gram 确定性 groundedness**（关键单变量改变）。

| metric | Original | Exp A | Exp B | Exp C | **Exp D** | Δ vs Exp C |
|---|---|---|---|---|---|---|
| escalation_rate | 85.2% | 85.6% | 92.0% | 93.0% | **67.2%** | **−25.8pp ↓↓** |
| normal_easy res | 23.2% | 24.4% | 11.3% | 6.5% | **40.5%** | **+34.0pp ↑↑** |
| multilingual res | 0% | 0% | 0% | 0% | **53.3%** | **+53.3pp ↑↑** |
| block_rate | 3.3% | 2.7% | 19.8% | 19.0% | 19.0% | 持平 |
| p50 / p95 latency | 3.4 / 5.2 s | (类似) | (类似) | (类似) | 5.1 / 8.4 s | +50% (含 judge call) |
| error_rate | 3.8% | 3.0% | 3.0% | (n/a) | 3.0% | 稳定 |

**§4h 假设被验证 ✓**：真瓶颈确实是确定性 n-gram groundedness 对英文 stiff template 回答的
false-fail；LLM-judge 用语义级判断把它修了——escalation 拉回 67%、normal_easy 解决率涨 34pp、
multilingual 从零起到 53%。

> **这是 3 轮假设证伪 + 第 4 次成功修复的完整研究闭环**：
> 1. ❌ KB 不够是瓶颈（§4g 证伪，扩到 176 篇无效）
> 2. ❌ critic 阈值 + PII 精度是瓶颈（§4h 证伪，校准+balanced 无效）
> 3. ✅ **groundedness check 是瓶颈，LLM-judge 修了**（Exp D 验证：escalation 拉低 26pp）
>
